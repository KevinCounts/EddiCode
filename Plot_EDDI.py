...
def getstakeholderconfig(stakeholder):
    ...
    match stakeholder:
        ...
        case 'USFS_NWOR':
            ...
            shape_list=os.listdir(shapedir+'MUSHRUSH_SeparateFDRAPolygons/')
            for file in shape_list:
                if file.endswith(".shp"):
                    shapefiles.append(shapedir+'MUSHRUSH_SeparateFDRAPolygons/'+file)
                    color.append('blue')
                    lw.append(.5)         
        case 'USGS_NCCSC':
...
def plotgen(latlon,shapefiles,color,lw,blncounties,path,dir_list,stakeholder):
    ...
    if blncounties:
        ax.add_feature(USCOUNTIES.with_scale('20m'),lw=.2,zorder = 6)
    ...
    fig.text(0.95,0.01,'Generated by NOAA/OAR/Physical Sciences Laboratory',fontsize=6,va='bottom',ha='right',color='dimgrey')
    ...
    for file in dir_list:
        if file.startswith("EDDI_ETrs_") and file.endswith(".asc"):
            infile = path + file
            ...
            cbar = fig.colorbar(conf,ax=ax,orientation='horizontal',ticks=clevs1,pad=.05)
            ...
            cbar.ax.text(-2.51,0.3,'ED4',fontsize=8,va='bottom',ha='center',color='white')
            cbar.ax.text(-1.85,0.3,'ED3',fontsize=8,va='bottom',ha='center',color='white')
            cbar.ax.text(-1.47,0.3,'ED2',fontsize=8,va='bottom',ha='center',color='white')
            cbar.ax.text(-1.06,0.3,'ED1',fontsize=8,va='bottom',ha='center',color='black')
            cbar.ax.text(-0.68,0.3,'ED0',fontsize=8,va='bottom',ha='center',color='black')
            cbar.ax.text(0.7,0.3,'EW0',fontsize=8,va='bottom',ha='center',color='black')
            cbar.ax.text(1.06,0.3,'EW1',fontsize=8,va='bottom',ha='center',color='black')
            cbar.ax.text(1.47,0.3,'EW2',fontsize=8,va='bottom',ha='center',color='white')
            cbar.ax.text(1.85,0.3,'EW3',fontsize=8,va='bottom',ha='center',color='white')
            cbar.ax.text(2.51,0.3,'EW4',fontsize=8,va='bottom',ha='center',color='white')
